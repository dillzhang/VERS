...
class FloorPlan extends Component {
  constructor(props) {
    ...
    if (this.props.level >= 40) {
      ...
      this.state = {
        ...
        dragX: -1,
        dragY: -1,
        interactivity: this.props.level < 39 && !this.props.host,
        holdingShift: false,
      };
    }
  ...
  handleMouseMove = (e) => {
    if (this.state.interactivity && this.state.dragging >= 0) {
      ...
      const best = sensorLocations
        .map((location, index) => {
          return [
            (this.state.dragX - this.xoffset + 15 - location[0]) ** 2 +
              (this.state.dragY + 15 - location[1]) ** 2,
            index,
          ];
        })
        .filter((distance) => distance[0] < 225)
        .sort((a, b) => a[0] - b[0]);
      const near = this.state.near.map(
        (_, i) => best.length > 0 && i === best[0][1]
      );
      this.setState((state) => ({
  ...
  handleMouseUp = (e) => {
    if (this.state.interactivity && this.state.dragging >= 0) {
      const sensors = this.state.sensors.map((v, i) =>
        this.state.near[i] ? this.state.dragging : v
      );
      ...
      this.setState(
        ...
        () => {
          this.props.socket.emit("sensorUpdate", {
  ...
  handleSensorClick = (e) => {
    ...
    const best = sensorLocations
      .map((location, index) => {
        return [
          (locX - this.xoffset + 15 - location[0]) ** 2 +
            (locY + 15 - location[1]) ** 2,
          index,
        ];
      })
      .filter((distance) => distance[0] < 900)
      .sort((a, b) => a[0] - b[0]);

    if (best.length > 0) {
      const dragging = this.state.holdingShift
        ? -1
        : this.state.sensors[best[0][1]];
      const sensors = this.state.sensors.map((v, i) =>
        (dragging !== -1 || this.state.holdingShift) && i === best[0][1]
          ? -1
          : v
      );
      const near = this.state.near.map((v, i) =>
        (dragging !== -1 || this.state.holdingShift) && i === best[0][1]
          ? true
          : v && this.state.holdingShift
      );
  ...
  render() {
    return (
      <div
        ...
        onMouseDown={this.handleSensorClick}
      >
        {!this.props.host && (
              ...
              <div
                ...
                onMouseDown={(e) => {
                  ...
                  this.setState({
                    dragging: 1,
                    dragX:
                      x - this.floorPlanRef.current.getBoundingClientRect().x,
                    dragY:
                      y - this.floorPlanRef.current.getBoundingClientRect().y,
                  });
                }}
              />
        ...
        <div className="sensor-container">
          ...
          >
            ...
            {laserPairs.map((pair, index) => {
              if (
                this.state.sensors[pair[0]] === 2 &&
                this.state.sensors[pair[1]] === 2
              ) {
                return (
                  <div
                    key={`laser-${index}`}
              ...
              return (
                <div
                  ...
                  className={`sensor sensor-direction-${location[2]} ${
                    this.state.near[index]
                      ? "near"
                      : `sensor-type-${
                          sensors[this.state.sensors[index]].class
                        }`
                  } ${
                    this.props.host
                      ? this.state.sensors[index] === correctSensors[index]
                        ? ""
                        : "incorrect-sensor"
                      : ""
                  }`}
        ...
        {this.state.dragging > -1 && (
          <div
            ...
            className={`dragging-sensor sensor-drag-${
              sensors[this.state.dragging].class
            }`}
          />
        )}
      </div>
